...
enum GoalType { AreaGoal,        /* subgoal (l) is met for given constant area
		...
		CachedGoal,      /* subgoal (l) is met, or has been met at least intData[0] times, within parent area (has the effect of caching evaluation of l, so after it's true for the first intData[0] times, it's never tested again) */
		RepeatGoal,      /* subgoal (l) is currently met & has been met *consecutively* at least intData[0] times within parent area (NB the consecutive requirement differs from CachedGoal) */
		...
		UseToolPseudoGoal,           /* calls useTool((Tool*)context,board,x,y,dblData[0]), where (x,y) is randomly sampled from parent area; returns true if Tool's reserve exhausted */
