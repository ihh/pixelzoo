...
and rewriting of bitfields in the local lattice neighborhood.
...
the core instructions are readily interpreted or compiled to C
(via {\tt switch}, {\tt if}, {\tt goto}, and integer math)
or directly to a RISC instruction set like ARM\cite{seal00} (via {\tt LDR}, {\tt STR}, {\tt ADD}, {\tt CMP}, {\tt B}).
\\
...
Scheme functions can be invoked dynamically by {\tt Call-Script} instruction
